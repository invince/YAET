...
External plugins are distributed as **directories** that users copy to `~/.yaet/plugins/<id>/`. There is no special archive format — just a folder with the correct structure.

#### Simple plugin (no build step)

For plugins like [`webdav-file-explorer`](../ext-plugins-example/webdav-file-explorer/) that have no npm dependencies and no frontend build step:
...
│   └── my.connector.js
└── frontend/
    └── index.js          # Hand-written Web Component (no build needed)
```

**Distribution**: Zip the directory and share. Users extract to `~/.yaet/plugins/my-plugin/`.

#### Plugin with npm dependencies (self-managed)

For plugins that need npm packages for the backend (e.g. `ssh2`, `telnet-client`):
...
For plugins that have frontend dependencies (e.g. a JS library like `spice-client`), you need a build script to bundle everything into a single `frontend/index.js`:

```
my-plugin/
├── package.json          # devDependencies for build, optional dependencies
├── build.js              # Build script (concatenates sources into frontend/index.js)
├── manifest.json
├── src/
│   ├── my-profile-form.js
│   └── my-session.js
...
│   └── my.connector.js
└── frontend/
    └── index.js          # Generated by build.js (do NOT hand-edit)
```

**Build script example** (from [`spice-remote-desktop/build.js`](../ext-plugins-example/spice-remote-desktop/build.js)):
...
const files = [
  path.join(SRC_DIR, 'my-profile-form.js'),
  path.join(SRC_DIR, 'my-session.js'),
];
...
// Add plugin metadata
bundle += '\nwindow.__MY_PLUGIN_PLUGIN__ = {\n';
bundle += '  manifest: {\n';
bundle += '    id: "my-plugin",\n';
bundle += '    name: "My Plugin",\n';
bundle += '    category: "TERMINAL",\n';
bundle += '    profileType: "MY_PLUGIN",\n';
bundle += '    supportedAuthTypes: ["N/A", "login", "secret"],\n';
bundle += '    secretTypes: ["LOGIN_PASSWORD"],\n';
bundle += '  },\n';
bundle += '  profileFormElement: "my-profile-form",\n';
bundle += '};\n';
...
Bundled plugins ship with the app and are compiled into the Angular bundle at build time. No separate packaging is needed — just place the plugin directory under `plugins/` and it is automatically discovered on the next `npm run build` or `npm start`.

The build script `scripts/generate-plugin-barrel.js` scans all `plugins/*/manifest.json` files and generates `plugins/generated-plugin-registry.ts`, which statically imports all plugin modules.
