...
function register(context) {
  ...
  const api = typeof context.runtimeAPI === 'function'
    ...
    : context.runtimeAPI;
  if (api) {
    api.registerConnector('SFTP_FILE_EXPLORER', (log, config) => {
      return new SftpFileExplorer(log, config);
    });
  }
  ...
  function getExplorer(configId) {
    const exp = sftpMap.get(configId);
    if (!exp) throw new Error('Error connection config not found');
    return exp;
  }

  ipcMain.handle('session.fe.sftp.register', async (event, { id, config, proxyId }) => {
    let sshConfig = { ...config };

    if (proxyId) {
      logger.info(`SFTP connection ${id}: Using proxy ${proxyId}`);
    ...
    const explorer = new SftpFileExplorer(logger, sshConfig);
    sftpMap.set(id, explorer);
  });

  app.post('/api/v1/sftp/download', downloadLimiter, async (req, res) => {
    ...
    try {
      ...
      const explorer = new SftpFileExplorer(logger, config);
  ...
  app.post('/api/v1/sftp/:id', generalLimiter, async (req, res) => {
    ...
    try {
      ...
      logger.error('Error handling SFTP request:', error);
      res.status(500).send({ error: { code: 500, message: error.message } });
    }
  });

  app.post('/api/v1/sftp/upload/:id', uploadLimiter, upload.single('uploadFiles'), async (req, res) => {
    let targetDir;
    try {
  ...
  app.post('/api/v1/sftp/download/:id', downloadLimiter, upload.none(), async (req, res) => {
    let downloadInput;
    try {
  ...
  app.post('/api/v1/sftp/open/:id', openLimiter, upload.none(), async (req, res) => {
    let downloadInput;
    try {
      downloadInput = JSON.parse(req.body.downloadInput);
    } catch (error) {
    ...
    try {
      ...
      const tempDir = path.join(os.tmpdir(), 'sftp-temp-files');
  ...
  logger.info('[sftp-file-explorer] Plugin registered');
}
